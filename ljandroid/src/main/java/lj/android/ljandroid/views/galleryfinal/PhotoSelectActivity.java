...
import lj.android.ljandroid.views.galleryfinal.adapter.FolderListAdapter;
import lj.android.ljandroid.views.galleryfinal.adapter.PhotoListAdapter;
import lj.android.ljandroid.views.galleryfinal.model.PhotoFolderInfo;
import lj.android.ljandroid.views.galleryfinal.model.PhotoInfo;
import lj.android.ljandroid.views.galleryfinal.permission.AfterPermissionGranted;
import lj.android.ljandroid.views.galleryfinal.permission.EasyPermissions;
import lj.android.ljandroid.views.galleryfinal.utils.PhotoTools;
 ...
 */
public class PhotoSelectActivity extends PhotoBaseActivity implements View.OnClickListener, AdapterView.OnItemClickListener,PhotoListAdapter.onChildItemClickListener {
	...
	private ArrayList<PhotoInfo> mSinglePhotoList=new ArrayList<PhotoInfo>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (GalleryFinal.getFunctionConfig() == null || GalleryFinal.getGalleryTheme() == null) {
			...
			mPhotoListAdapter.setOnChildItemClickListener(this);
...
//				mIvTakePhoto.setVisibility(View.VISIBLE);
	...
	/*	mIvBack.setImageResource(GalleryFinal.getGalleryTheme().getIconBack());
		if (GalleryFinal.getGalleryTheme().getIconBack() == R.mipmap.secretary_shape) {
		...
		if (GalleryFinal.getGalleryTheme().getIconFolderArrow() == R.mipmap.ic_gf_triangle_arrow) {
		...
		if (GalleryFinal.getGalleryTheme().getIconClear() == R.mipmap.ic_gf_clear) {
		...
		if (GalleryFinal.getGalleryTheme().getIconPreview() == R.mipmap.gf_ic_preview) {
		...
		if (GalleryFinal.getGalleryTheme().getIconCamera() == R.mipmap.ic_gf_camera) {
	...
	private void takeRefreshGallery(PhotoInfo photoInfo) {
		...
		mPhotoListAdapter.notifyDataSetChanged();
		if (mAllPhotoFolderList.size() == 0) return;
		//添加到集合中
...
//                        mCurPhotoList.addAll(allFolderList.get(0).getPhotoList());
						/************************     针对第一张显示 需求做了 改变****************************/
						List<PhotoInfo> photoList = allFolderList.get(0).getPhotoList();
						photoList.add(0, getFirstPhoto());
						mCurPhotoList.addAll(photoList);
					}
				}
	...
	private PhotoInfo getFirstPhoto() {
		PhotoInfo firstPhoto = new PhotoInfo();
		firstPhoto.setPhotoId(0);
		firstPhoto.setPhotoPath("res:///" + R.mipmap.choosefilm_photo);
		return firstPhoto;
	}
	...
	@Override
	public void ivCheckClick(View view,int position) {
		photoItemClick(view,position);
	}


	@Override
	public void ivPreViewClick( PhotoInfo photoInfo,View view,int position) {
		if(GalleryFinal.getFunctionConfig().isMutiSelect()){
			if(mSinglePhotoList!=null&&!mSinglePhotoList.contains(photoInfo)){
				mSinglePhotoList.clear();
				mSinglePhotoList.add(photoInfo);
			}
			Intent intent = new Intent(this, PhotoPreviewActivity.class);
			intent.putExtra(PhotoPreviewActivity.PHOTO_LIST, mSinglePhotoList);
			startActivity(intent);
		}else{
			if(position==0){
				if (GalleryFinal.getFunctionConfig().isMutiSelect() && GalleryFinal.getFunctionConfig().getMaxSize() == mSelectPhotoList.size()) {
